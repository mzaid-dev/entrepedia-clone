...
const MasterLibrary = () => {
    return (
        <section className="relative w-full bg-black pt-40 pb-24 px-6 z-20 text-white overflow-hidden border-t border-white/5">
            {/* Top Logo Cloud */}
            <div className="max-w-7xl mx-auto text-center mb-20">
                <p className="text-[0.7rem] font-bold tracking-[0.2em] uppercase opacity-60 mb-10 text-white">Trusted by top brands and individuals worldwide</p>
                <div className="flex flex-wrap justify-center items-center gap-12 md:gap-20 opacity-50 grayscale hover:grayscale-0 transition-all duration-500">
                    <span className="text-2xl font-black tracking-tighter text-white">CPHERE</span>
                    <div className="flex flex-col items-center leading-none">
                        <span className="text-[0.6rem] font-bold border border-white/40 px-1 mb-1 text-white">CEO</span>
                        <span className="text-xl font-bold tracking-tighter italic text-white">MasterMind</span>
                    </div>
                    <div className="flex items-center gap-2">
                        <div className="w-6 h-6 border-2 border-white/60 flex items-center justify-center font-bold text-xs text-white">+</div>
                        <span className="text-lg font-bold text-white">Local Pulse</span>
                    </div>
                    <div className="flex items-center gap-1">
                        <span className="text-2xl font-black italic text-white">Donna</span>
                        <span className="text-[0.6rem] font-bold opacity-60 mb-3 ml-0.5 text-white">Pro</span>
                    </div>
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto text-center mb-24">
                ...
                <h2 className="text-[2.5rem] md:text-[4rem] lg:text-[5rem] leading-[1.1] font-black text-white mb-10 tracking-tight">
                    Sell or use market-ready <br />
                    <span className="text-accent-red">products under your name</span>
                </h2>
                <p className="text-lg md:text-xl text-white/50 font-medium mb-12 max-w-2xl mx-auto leading-relaxed">
                ...
                <button className="bg-white text-black px-10 py-5 rounded-full font-black text-lg flex items-center gap-4 mx-auto hover:bg-gray-200 transition-all active:scale-95 shadow-2xl">
                    <div className="bg-black text-white p-1 rounded-full"><Play fill="currentColor" size={20} /></div>
                    Watch Explainer Video
                </button>
            </div>

            {/* Platform Mockup Placeholder */}
            <div className="max-w-6xl mx-auto mb-32 relative px-4 md:px-0">
                <div className="bg-white/5 backdrop-blur-3xl rounded-[40px] border border-white/10 p-4 shadow-2xl overflow-hidden aspect-[16/9] md:aspect-[21/9]">
            ...
            <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-8 mb-32">
                <FeatureCard title="Demanded Content" desc="Best-selling topics that people really buy. Based on market research." color="bg-white/5 text-white" />
                ...
                <FeatureCard title="1000+" subtitle="Digital Products" desc="Almost unlimited selection of content in one of the biggest libraries out there." color="bg-[#111] text-white" variant="dark" />
                <FeatureCard title="15 Media Formats" desc="Ebooks, Courses, Audios, Templates, Prompts, Notion Systems + more." color="bg-white/5 text-white" />
                <FeatureCard title="Constantly Updated" desc="We add 20 new products every week to meet the latest trends." color="bg-white/5 text-white" />
                <FeatureCard title="Created by Experts" desc="Each product is made in-house by our team. World-class standards." color="bg-white/5 text-white" />
            </div>

            {/* Usage Options Section */}
            <div className="max-w-4xl mx-auto text-center mb-20">
                <div className="inline-block bg-white/10 text-white text-[0.7rem] font-black uppercase px-6 py-2 rounded-full mb-8">
                ...
                <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 px-4 md:px-0">
...
const FeatureCard = ({ title, subtitle, desc, color, variant }) => (
    <div className={`${color} p-10 rounded-[40px] border border-white/10 shadow-lg flex flex-col justify-start h-full min-h-[350px] transition-transform hover:-translate-y-2 relative overflow-hidden`}>
        {variant === 'large' && <div className="absolute -bottom-20 -left-10 w-60 h-20 bg-black/10 blur-2xl rotate-45" />}
        {variant === 'dark' && <div className="mb-6 opacity-30"><ShoppingBag size={80} /></div>}
        ...
        <p className="text-md opacity-60 font-medium leading-relaxed">{desc}</p>
    </div>
);
...
const UsageItem = ({ icon, text }) => (
    <div className="bg-white/5 border border-white/10 p-8 rounded-[30px] flex flex-col items-center justify-center gap-6 hover:bg-white/10 transition-all group">
        ...
        <span className="text-[0.8rem] font-bold text-white/80 leading-tight">{text}</span>
    </div>
);
